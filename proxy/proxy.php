...
class Proxy {
    ...
    */
	...
	//ADAPTED version - see original at https://github.com/Esri/resource-proxy/blob/master/PHP/proxy.php
    public function isAllowedApplication()
    {
		...
		foreach($this->proxyConfig['allowedreferers'] as $allowed) {
			
			if(preg_match("/^" . addcslashes($allowed, "/") ."/", $this->referer)){  //To allow http://domain.com/* wildcards using regexp
		...
		$this->proxyLog->log("Attempt made to use this proxy from " . $this->referer . " and " . $_SERVER['REMOTE_ADDR']. " and " . $_SERVER['HTTP_REFERER']. " autorizados:" . $this->proxyConfig['allowedreferers']);
